...
void main() {
  ...
  test('value lists share one canonical immutable JSON representation', () {
    final collectionSpec = EntitySpec(
      className: spec.className,
      packageName: spec.packageName,
      inputImport: spec.inputImport,
      tableName: spec.tableName,
      ...
      fields: [
        ...
        const FieldSpec(
          ...
          dartType: 'List<ReminderInterval>',
          ...
          indexed: false,
          unique: false,
          enumValues: ['oneDay', 'twoDays'],
          collectionElementDartType: 'ReminderInterval',
          collectionElementSqlType: SqlType.text,
        ),
        ...spec.fields.skip(4),
      ],
      security: spec.security,
    ...
    final dart = emitDart(collectionSpec);
    expect(dart, contains('EntityFieldKind.list'));
    expect(dart, contains('elementKind: EntityFieldKind.text'));
    expect(dart, contains("protocolDefault: const ['one_day']"));
    expect(
      dart,
      contains(
    ...
    expect(dart, contains('immutableEntityList<ReminderInterval>(value)'));
    expect(dart, contains('entityValuesEqual(oldValue, value)'));
    expect(
      dart.indexOf('entityValuesEqual(oldValue, value)'),
      lessThan(dart.indexOf('immutableEntityList<ReminderInterval>(value)')),
    );
    expect(dart, contains('decodeEntityList<ReminderInterval>'));
    expect(dart, contains('json_valid(reminders)'));
    expect(dart, contains(r'''const Constant('["one_day"]')'''));

    final sql = emitSupabaseSql(collectionSpec);
    expect(
      sql,
      contains("reminders jsonb not null default '[\"one_day\"]'::jsonb"),
    );
    expect(sql, contains("jsonb_typeof(reminders) = 'array'"));
    expect(sql, contains(r'''@.type() != "string"'''));
    expect(sql, contains(r'''@ != "one_day"'''));
    expect(
      sql,
      contains(
        "case when jsonb_typeof(current_operation -> 'patch' -> "
        "'reminders') = 'null' then null else current_operation -> "
        "'patch' -> 'reminders' end",
      ),
    );
  });

  test('structured values share one inferred JSON codec and root shape', () {
    final jsonSpec = EntitySpec(
      className: spec.className,
      packageName: spec.packageName,
      inputImport: spec.inputImport,
      tableName: spec.tableName,
      ...
      fields: [
        ...
        const FieldSpec(
          ...
          indexed: false,
          unique: false,
          jsonValue: JsonValueSpec(shape: JsonValueShape.object),
        ),
        ...spec.fields.skip(4),
      ],
      security: spec.security,
    ...
    final dart = emitDart(jsonSpec);
    expect(dart, contains('kind: EntityFieldKind.json'));
    expect(dart, contains('jsonShape: JsonValueShape.object'));
    expect(dart, contains('Metadata.fromJson(canonicalJsonObject('));
    expect(dart, contains('canonicalJsonObject(value.toJson()'));
    expect(dart, contains('entityValuesEqual(oldValue, value)'));
    expect(dart, contains('final normalizedValue = Metadata.fromJson('));
    expect(dart, contains('json_valid(metadata)'));

    final sql = emitSupabaseSql(jsonSpec);
    expect(sql, contains('metadata jsonb not null'));
    expect(sql, contains("jsonb_typeof(metadata) = 'object'"));
    expect(
      sql,
      contains(
        "case when jsonb_typeof(current_operation -> 'patch' -> "
        "'metadata') = 'null' then null else current_operation -> "
        "'patch' -> 'metadata' end",
      ),
    );
  });
